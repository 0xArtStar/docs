...
<Accordion title="4. Standards, EIPs, and Base Components" icon="cube">
  <Tabs>
    <Tab title="ERC-20 & SafeERC20">
      The contract keeps the standard token interface, while layering additional
      business restrictions on transfer and mint paths. It uses `SafeERC20` to
      wrap `transferFrom` and `transfer` calls for more robust interactions and
      reduced integration risk.
    </Tab>
    <Tab title="ERC-1967 & UUPS">
      `ERC-1967` defines the storage slot conventions used by proxy contracts.
      This project uses the `UUPSUpgradeable` pattern, placing the upgrade
      entrypoint and authorization logic in the implementation contract, with
      `_authorizeUpgrade()` determining upgrade permissions.
    </Tab>
    <Tab title="Governance and Pausing">
      - `Initializable`: prevents accidental initialization of the
      implementation contract. - `OwnableUpgradeable`: a single-admin governance
      model, with authority concentrated in the owner. - `PausableUpgradeable`:
      emergency pause capability affecting both primary sales and secondary
      transfers.
    </Tab>
    <Tab title="Security and Math">
      - `ReentrancyGuardTransient`: additional reentrancy protection for
      `mint()`. - `Math.mulDiv`: precise conversion between `USDT` amounts and
      18-decimal token amounts. - `__gap`: reserved storage slots for future
      state variables, reducing the risk of breaking storage layout
      compatibility.
    </Tab>
  </Tabs>
</Accordion>
...
<Steps>
  <Step title="Operational configuration">
    The owner sets price and purchase bounds, configures the sale window, and
    calls `startSale()`.
  </Step>
  ...
  <Step title="Compliance verification">
    The contract checks the sale window, per-transaction limits, and the hard
    cap, then verifies the buyer’s authorization via `ComplianceManage`.
  </Step>
  <Step title="Payment and minting">
    After verification, `USDT` is transferred from the buyer to `custodyWallet`.
    The contract updates state and mints tokens to the buyer.
  </Step>
</Steps>
